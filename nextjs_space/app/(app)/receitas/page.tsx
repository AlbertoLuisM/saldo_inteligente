...
const CATEGORIES = ['Salário','Freelance','Negócios','Renda Extra','Renda Passiva','Outros'];
...
interface Income {
  ...
  date: string; recurrence: string; notes: string | null;
}

const emptyForm = { description:'', amount:'', category:'Salário', date: formatDateInput(new Date()), recurrence:'once', notes:'' };
...
export default function ReceitasPage() {
  ...
  const load = useCallback(() => {
    ...
    fetch(`/api/incomes${q}`).then(r => r.json()).then(d => { setItems(Array.isArray(d) ? d : []); setLoading(false); }).catch(() => setLoading(false));
  }, [filterCat]);

  useEffect(() => { load(); }, [load]);

  function openNew() { setEditing(null); setForm(emptyForm); setShowModal(true); }
  function openEdit(item: Income) { setEditing(item); setForm({ description: item.description, amount: String(item.amount), category: item.category, date: formatDateInput(item.date), recurrence: item.recurrence, notes: item.notes ?? '' }); setShowModal(true); }
  ...
  return (
    <div style={{ fontFamily: 'Inter, sans-serif' }}>
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        <div>
          <h1 className="text-lg sm:text-xl font-bold text-gray-800 dark:text-gray-100">Receitas</h1>
          <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 dark:text-gray-400 dark:text-gray-500 mt-0.5">Gerencie suas fontes de renda</p>
        </div>
        <button onClick={openNew} className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg text-white text-xs sm:text-sm font-semibold" style={{ background: '#10b981' }}>
          <Plus className="w-4 h-4" /> <span className="hidden sm:inline">Nova</span> Receita
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-white dark:bg-gray-800/60 rounded-xl p-4 border border-gray-100 dark:border-gray-700/50 shadow-sm">
          <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 dark:text-gray-400 dark:text-gray-500">Total do período</p>
          <p className="text-2xl font-bold mt-1" style={{ color: '#10b981' }}>{formatCurrency(total)}</p>
        </div>
        <div className="bg-white dark:bg-gray-800/60 rounded-xl p-4 border border-gray-100 dark:border-gray-700/50 shadow-sm">
          <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 dark:text-gray-400 dark:text-gray-500">Quantidade de receitas</p>
          <p className="text-2xl font-bold mt-1 text-gray-800 dark:text-gray-100">{items.length}</p>
        </div>
        <div className="bg-white dark:bg-gray-800/60 rounded-xl p-4 border border-gray-100 dark:border-gray-700/50 shadow-sm">
          <p className="text-sm text-gray-500 dark:text-gray-400 dark:text-gray-500 dark:text-gray-400 dark:text-gray-500">Média por receita</p>
      ...
      <div className="bg-white dark:bg-gray-800/60 rounded-xl border border-gray-100 dark:border-gray-700/50 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <EmptyState icon={TrendingUp} title="Nenhuma receita" description="Adicione suas fontes de renda para começar o controle financeiro." action={<button onClick={openNew} className="px-4 py-2 rounded-lg text-white text-sm" style={{ background: '#10b981' }}>Adicionar Receita</button>} />
        ) : (
          <>
            {/* Desktop table */}
            <table className="w-full hidden md:table">
              <thead className="border-b border-gray-100 dark:border-gray-700/50">
                <tr>
                  {['Descrição','Categoria','Data','Recorrência','Valor','Ações'].map(h => (
                    <th key={h} className="text-left py-3 px-4 text-xs font-semibold text-gray-500 dark:text-gray-400 dark:text-gray-500 dark:text-gray-400 dark:text-gray-500 uppercase tracking-wide">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {items.map(item => (
                  <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50 dark:bg-gray-900/50 transition-colors">
                    ...
                    <td className="py-3 px-4"><span className="text-xs px-2 py-0.5 rounded-full bg-green-50 text-green-700">{item.category}</span></td>
                    ...
                    <td className="py-3 px-4"><StatusBadge status={item.recurrence} /></td>
              ...
              {items.map(item => (
                <div key={item.id} className="p-4">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className="text-xs px-2 py-0.5 rounded-full bg-green-50 text-green-700">{item.category}</span>
                  ...
                  <div className="flex items-center justify-between mt-2">
                    <StatusBadge status={item.recurrence} />
                    <div className="flex items-center gap-2">
                      <button onClick={() => openEdit(item)} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700/50 text-gray-400 dark:text-gray-500 dark:text-gray-400 dark:text-gray-500"><Pencil className="w-3.5 h-3.5" /></button>
                      <button onClick={() => handleDelete(item.id)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/30 text-gray-400 dark:text-gray-500 dark:text-gray-400 dark:text-gray-500 hover:text-red-500"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
      ...
      {/* Modal */}
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editing ? 'Editar Receita' : 'Nova Receita'}>
        <div className="space-y-4">
          <FormField label="Descrição *">
            <input className={inputClass} value={form.description} onChange={e => setForm(p => ({...p, description: e.target.value}))} placeholder="Ex: Salário mensal" />
          </FormField>
